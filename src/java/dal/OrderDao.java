...
public class OrderDao extends DBConnect {
    ...
    public List<Order> getAll(String userId) {
        ...
        String sql = "select * from [Order] o join \n"
                + "(select distinct od.orderStatus, od.orderId, A.productName, A.image, od.boughtQuantity, A.color, A.price, A.size from OrderDetail od join \n"
                + "(select p.productName, p.image, td.teddyId, td.color, td.price, td.size from Product p left join TeddyDetail td on p.productId = td.productId) \n"
                + "as A on od.teddyId = A.teddyId) \n"
                + "as B on o.orderId = B.orderId where o.userId = ?";
        try {
