...
public class CartServlet extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        if (id != null && image != null && name != null && size != null && color != null && price != null && quantity != null) {
            ...
            if (txt.isEmpty()) {
                txt = cartItem; // nếu rỗng thì thêm mới
            } else {
                ...
                for (String item : items) {
                    ...
                    if (details.length == 7) {
                        ...
                        // Kiểm tra xem sản phẩm đã tồn tại (cùng id, size, color)
                        if (existingId.equals(id) && existingSize.equals(size) && existingColor.equals(color)) {
                            ...
                            newCart.append(item); // giữ lại các sản phẩm khác
                        }
                        newCart.append(",");
                    }
                }

                // thêm mới nếu chưa tồn tại sản phẩm
                if (!productExists) {
