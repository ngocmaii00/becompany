    ...
    <body>
        ...
        <div>
            <span class="grid grid-cols-11 font-bold text-xl items-center mx-16 text-[#543520] py-4">
                <input type="checkbox" class="col-span-1 size-5 mx-12 accent-[#543520]" />
                ...
                if (cookies != null) {
                    for (Cookie cookie : cookies) {
                        if ("cart".equals(cookie.getName())) {
                            cartData = java.net.URLDecoder.decode(cookie.getValue(), "UTF-8");
                            break;
                        }
                    }
                }
                pageContext.setAttribute("cartData", cartData);
            %>

            <c:if test="${not empty cartData}">
                <div class="border-2 border-[#543520] mx-16 rounded-lg py-4 bg-[#f2e6e6]">
                    <c:forEach var="item" items="${fn:split(cartData, ',')}" varStatus="loop">
                            ...
                            <div class="grid grid-cols-11 items-center py-4">
                                ...
                                <div class="col-span-3">
                                    <div class="flex flex-row place-items-center gap-x-6">
                                        <div class="rounded-lg border-2 border-[#543520] size-28 bg-cover bg-center" style="background-image: url('${parts[0]}');"></div>
                                        <div>
                                            <span class="font-bold text-3xl text-[#543520]">${parts[1]}</span>
                                            <div class="flex flex-row items-center gap-x-2 mt-3">
                                                <div class="border-2 w-fit border-[#543520] rounded-full px-2">${parts[2]}</div>
                                                <div class="rounded-full size-6" style="background-color: ${parts[3]};"></div>
                                            </div>
                                        </div>
                                    </div>
                                </div>
                                ...
                                <div class="col-span-2 place-items-center">
                                    <div class="flex flex-row border-2 border-black h-fit w-fit rounded-md col-span-2 items-center mx-auto">
                                        <button onclick="updateQuantity('${parts[1]}', 1)">
                                        ...
                                        <input type="text" value="${parts[5]}" class="bg-transparent outline-none w-8 text-center" readonly />
                                        <button onclick="updateQuantity('${parts[1]}', -1)">
                                            <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="#543520" class="size-6">
            ...
            <c:if test="${empty cartData}">
                <div class="text-center text-[#543520] font-bold text-xl mx-16 py-4">Your cart is empty!</div>
            </c:if>
            ...
            <%
                ...
                pageContext.setAttribute("itemCount", cartData.isEmpty() ? 0 : cartData.split(",").length - (cartData.endsWith(",") ? 1 : 0));
            %>

            <div class="grid grid-cols-11 mx-16 items-center my-4 h-14">
                <div class="col-span-7 rounded-lg size-6 h-11 w-20 bg-[#543520] flex flex-row items-center space-x-4 justify-between">
                    <button class="relative group mx-2 flex flex-row space-x-4 justify-between">
                ...
                <div class="col-span-3">
                    <input type="text" id="voucher" name="voucher" placeholder="Voucher" class="border-black border-2 h-11 rounded-lg w-30 placeholder-grey-500 pl-2 font-bold" />
                </div>
                <div class="col-span-1 rounded-lg h-11 w-28 bg-[#543520] flex flex-row items-center justify-center">
                    <button class="relative group">
                        <h3 class="text-white text-lg font-bold text-center items-center">Apply</h3>
                    </button>
                </div>
            </div>
        ...
        <script>
            function updateQuantity(name, change) {
                // Logic to update quantity in cookie and refresh page
                // This requires a servlet call to update the cookie
                // For simplicity, this is a placeholder
                alert("Update quantity for " + name + " by " + change);
            }
        </script>
    </body>
</html>